...
/**
* @b0o0wen/dsh-headless-resume — one-shot direct Agent driver. The bundle patch
* rides over dsh-base without Host, HTTP, or browser plugins; this runner
* creates or resumes one Agent through the core registry, drives the task to quiescence,
* streams provider reasoning to stderr, flushes its Session, prints the final
* assistant text to stdout, and exits.
*
* @module @b0o0wen/dsh-headless-resume
*/
...
/**
* Run one task through a new or persisted Agent and request process exit.
* @param ctx - plugin context carrying the Agent, default model, Session, and launcher IO services.
* @param config - task text and the exact persisted session ID, if resuming.
...
async function run(ctx, config, io) {
	...
	let agent;
	if (config.resume) {
		var rid = config.resume.startsWith("session-") ? config.resume : `session-${config.resume}`;
		io.stderr.write(`dsh: resuming session ${rid}\n`);
		const r = await agents.resume({ ...base, resumeSessionId: brandString(rid) });
		agent = r.agent;
	} else {
	...
	io.stderr.write(`dsh: session: ${agent.session.id}\n`);
