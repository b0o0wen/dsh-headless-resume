...
const Config = z.object({ task: z.string().required(), resume: z.string() });
...
function sessionUuid(id) { return String(id).replace(/^session-/, ""); }
...
async function run(ctx, task, io) {
	...
	let agent;
	var _startup = ctx.get("headlessStartup") || {};
	var resumeId = config.resume || _startup.resume || "";
	if (resumeId) {
		io.stderr.write("dsh: resuming session " + resumeId + "\n");
		const r = await agents.resume({ ...base, resumeSessionId: brandString(resumeId) });
		agent = r.agent;
	} else {
		const c = await agents.create({ ...base, sessionId: brandString(`session-${randomUUID()}`) });
		agent = c.agent;
	}
